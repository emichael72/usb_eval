...
#define binding_to_mmio(b) container_of(b, struct mctp_binding_astlpc_mmio, astlpc)

static int mctp_astlpc_mmio_kcs_read(void *data, enum mctp_binding_astlpc_kcs_reg reg, uint8_t *val)
{
...
int mctp_astlpc_mmio_lpc_write(void *data, const void *buf, long offset, size_t len)
{
    struct mctp_binding_astlpc_mmio *mmio = binding_to_mmio(data);
...
static void astlpc_test_rx_message(uint8_t eid __unused, bool tag_owner __unused, uint8_t msg_tag __unused, void *data __unused, void *msg, size_t len)
{
...
static int endpoint_init(struct astlpc_endpoint *ep, mctp_eid_t eid, uint8_t mode, uint32_t mtu, uint8_t (*kcs)[2], void *lpc_mem)
{
    /*
    ...
    ep->astlpc = mctp_astlpc_init(mode, mtu, lpc_mem, &astlpc_direct_mmio_ops, &ep->mmio);
    assert(ep->astlpc);
...
static void network_init(struct astlpc_test *ctx)
{
    ...
    /* BMC initialisation */
    rc = endpoint_init(&ctx->bmc, 8, MCTP_BINDING_ASTLPC_MODE_BMC, MCTP_BTU, &ctx->kcs, ctx->lpc_mem);
    assert(! rc);
    assert(ctx->kcs[MCTP_ASTLPC_KCS_REG_STATUS] & KCS_STATUS_BMC_READY);

    /* Host initialisation */
    rc = endpoint_init(&ctx->host, 9, MCTP_BINDING_ASTLPC_MODE_HOST, MCTP_BTU, &ctx->kcs, ctx->lpc_mem);
    assert(! rc);

    /* BMC processes host channel init request, alerts host */
    mctp_astlpc_poll(ctx->bmc.astlpc);
    assert(ctx->kcs[MCTP_ASTLPC_KCS_REG_STATUS] & KCS_STATUS_CHANNEL_ACTIVE);
...
static void astlpc_test_packetised_message_bmc_to_host(void)
{
    ...
    /* BMC sends a message */
    rc = mctp_message_tx(ctx.bmc.mctp, 9, MCTP_MESSAGE_TO_SRC, 0, msg, sizeof(msg));
    assert(rc == 0);
...
static void astlpc_test_simple_message_host_to_bmc(void)
{
    ...
    rc = mctp_message_tx(ctx.host.mctp, 8, MCTP_MESSAGE_TO_DST, tag, msg, sizeof(msg));
    assert(rc == 0);
...
static void astlpc_test_simple_message_bmc_to_host(void)
{
    ...
    rc = mctp_message_tx(ctx.bmc.mctp, 9, MCTP_MESSAGE_TO_SRC, tag, msg, sizeof(msg));
    assert(rc == 0);
    assert(ctx.kcs[MCTP_ASTLPC_KCS_REG_STATUS] & KCS_STATUS_OBF);
    assert(ctx.kcs[MCTP_ASTLPC_KCS_REG_DATA] == 0x01);
...
static void astlpc_test_host_before_bmc(void)
{
    ...
    /* Initialise the binding */
    astlpc = mctp_astlpc_init(MCTP_BINDING_ASTLPC_MODE_HOST, MCTP_BTU, NULL, &astlpc_direct_mmio_ops, &mmio);
...
{
    assert(0 == mctp_astlpc_negotiate_version(ASTLPC_VER_BAD, ASTLPC_VER_CUR, ASTLPC_VER_MIN, ASTLPC_VER_CUR));
    assert(0 == mctp_astlpc_negotiate_version(ASTLPC_VER_MIN, ASTLPC_VER_BAD, ASTLPC_VER_MIN, ASTLPC_VER_CUR));
    assert(0 == mctp_astlpc_negotiate_version(ASTLPC_VER_MIN, ASTLPC_VER_CUR, ASTLPC_VER_BAD, ASTLPC_VER_CUR));
    assert(0 == mctp_astlpc_negotiate_version(ASTLPC_VER_MIN, ASTLPC_VER_CUR, ASTLPC_VER_MIN, ASTLPC_VER_BAD));
    assert(0 == mctp_astlpc_negotiate_version(ASTLPC_VER_CUR + 1, ASTLPC_VER_CUR, ASTLPC_VER_MIN, ASTLPC_VER_CUR + 1));
    assert(0 == mctp_astlpc_negotiate_version(ASTLPC_VER_MIN, ASTLPC_VER_CUR + 1, ASTLPC_VER_CUR + 1, ASTLPC_VER_CUR));
}

static void astlpc_test_incompatible_versions(void)
{
    assert(0 == mctp_astlpc_negotiate_version(ASTLPC_VER_CUR, ASTLPC_VER_CUR, ASTLPC_VER_CUR + 1, ASTLPC_VER_CUR + 1));
    assert(0 == mctp_astlpc_negotiate_version(ASTLPC_VER_CUR + 1, ASTLPC_VER_CUR + 1, ASTLPC_VER_CUR, ASTLPC_VER_CUR));
}
...
static void astlpc_test_version_host_fails_negotiation(void)
{
    ...
    /* BMC initialisation */
    rc = endpoint_init(&bmc, 8, MCTP_BINDING_ASTLPC_MODE_BMC, MCTP_BTU, &kcs, lpc_mem);
    assert(! rc);
    ...
    /* Host initialisation */
    rc = endpoint_init(&host, 9, MCTP_BINDING_ASTLPC_MODE_HOST, MCTP_BTU, &kcs, lpc_mem);
    assert(rc < 0);
...
static void astlpc_test_simple_indirect_message_bmc_to_host(void)
{
    ...
    ctx.bmc.astlpc        = mctp_astlpc_init(MCTP_BINDING_ASTLPC_MODE_BMC, MCTP_BTU, NULL, &astlpc_indirect_mmio_ops, &ctx.bmc.mmio);
    ...
    ctx.host.astlpc        = mctp_astlpc_init(MCTP_BINDING_ASTLPC_MODE_HOST, MCTP_BTU, NULL, &astlpc_indirect_mmio_ops, &ctx.host.mmio);
...
static void astlpc_test_buffers_bad_host_init(void)
{
    ...
    host.astlpc = mctp_astlpc_init(MCTP_BINDING_ASTLPC_MODE_HOST, 0, lpc_mem, &astlpc_direct_mmio_ops, &host.mmio);
...
static void astlpc_test_negotiate_increased_mtu(void)
{
    ...
    /* BMC initialisation */
    rc = endpoint_init(&bmc, 8, MCTP_BINDING_ASTLPC_MODE_BMC, 3 * MCTP_BTU, &kcs, lpc_mem);
    assert(! rc);

    /* Host initialisation */
    rc = endpoint_init(&host, 9, MCTP_BINDING_ASTLPC_MODE_HOST, 2 * MCTP_BTU, &kcs, lpc_mem);
    assert(! rc);
...
static void astlpc_test_negotiate_mtu_low_high(void)
{
    ...
    /* BMC initialisation */
    bmtu = 3 * MCTP_BTU;
    rc   = endpoint_init(&bmc, 8, MCTP_BINDING_ASTLPC_MODE_BMC, bmtu, &kcs, lpc_mem);
    assert(! rc);

    /* Host initialisation with low MTU */
    hmtu = 2 * MCTP_BTU;
    rc   = endpoint_init(&host, 9, MCTP_BINDING_ASTLPC_MODE_HOST, hmtu, &kcs, lpc_mem);
    assert(! rc);
    ...
    assert(host.astlpc->layout.rx.size == astlpc_layout_ctx.proto->packet_size(MCTP_PACKET_SIZE(hmtu)));
    ...
    assert(host.astlpc->layout.rx.size == astlpc_layout_ctx.proto->packet_size(MCTP_PACKET_SIZE(bmtu)));
...
static void astlpc_test_send_large_packet(void)
{
    ...
    /* BMC initialisation */
    rc = endpoint_init(bmc, 8, MCTP_BINDING_ASTLPC_MODE_BMC, 8192, &kcs, lpc_mem);
    assert(! rc);

    /* Host initialisation */
    rc = endpoint_init(host, 9, MCTP_BINDING_ASTLPC_MODE_HOST, 8192, &kcs, lpc_mem);
    assert(! rc);
    ...
    rc = mctp_message_tx(host->mctp, 8, MCTP_MESSAGE_TO_DST, tag, ctx.msg, 2 * MCTP_BODY_SIZE(8192));
    assert(rc == 0);
    rc = mctp_astlpc_poll(bmc->astlpc);
    assert(rc == 0);
    rc = mctp_astlpc_poll(host->astlpc);
...
static void astlpc_test_negotiate_mtu_high_low(void)
{
    ...
    /* BMC initialisation */
    bmtu = 3 * MCTP_BTU;
    rc   = endpoint_init(&ctx.bmc, 8, MCTP_BINDING_ASTLPC_MODE_BMC, bmtu, &ctx.kcs, ctx.lpc_mem);
    assert(! rc);

    /* Host initialisation with low MTU */
    hmtu = 3 * MCTP_BTU;
    rc   = endpoint_init(&ctx.host, 9, MCTP_BINDING_ASTLPC_MODE_HOST, hmtu, &ctx.kcs, ctx.lpc_mem);
    assert(! rc);
	 ...
	 */
    rc = mctp_message_tx(ctx.bmc.mctp, 9, MCTP_MESSAGE_TO_DST, tag, msg, sizeof(msg));
    ...
    rc = mctp_message_tx(ctx.bmc.mctp, 9, MCTP_MESSAGE_TO_DST, tag, msg, hmtu);
...
static void astlpc_test_tx_before_channel_init(void)
{
    ...
    /* BMC initialisation */
    rc = endpoint_init(bmc, 8, MCTP_BINDING_ASTLPC_MODE_BMC, 0, &kcs, lpc_mem);
    assert(! rc);
	 ...
	 */
    alarm(1);
    mctp_message_tx(bmc->mctp, 9, MCTP_MESSAGE_TO_SRC, tag, msg, sizeof(msg));
    alarm(0);
...
static void astlpc_test_async_exchange(void)
{
	 ...
	 */
    mctp_message_tx(ctx.bmc.mctp, 9, MCTP_MESSAGE_TO_SRC, tag, msg, sizeof(msg));
	 ...
	 */
    mctp_message_tx(ctx.host.mctp, 8, MCTP_MESSAGE_TO_SRC, tag, msg, sizeof(msg));
    mctp_astlpc_poll(ctx.bmc.astlpc);
