...
static void test_rx(uint8_t eid __unused, bool tag_owner __unused, uint8_t msg_tag __unused, void *data, void *msg, size_t len)
{
...
static void run_one_test(struct test_ctx *ctx, struct test *test)
{
    ...
    for ( i = 0; i < (int) ctx->rx_len; i++ ) assert(ctx->rx_data[i] == i);
...
int main(void)
{
    ...
    for ( i = 0; i < ARRAY_SIZE(tests); i++ ) run_one_test(&ctx, &tests[i]);
