...
static void rx_message(uint8_t eid __unused, bool tag_owner, uint8_t msg_tag, void *data, void *msg __unused, size_t len)
{
 ...
 * provide.
 */
static void receive_ptkbuf(struct mctp_binding_test *binding, const struct pktbuf *pktbuf, size_t len)
{
...
static void receive_one_fragment(struct mctp_binding_test *binding, uint8_t *payload, size_t fragment_size, uint8_t flags_seq_tag, struct pktbuf *pktbuf)
{
...
static void receive_two_fragment_message(struct mctp_binding_test *binding, uint8_t *payload, size_t fragment1_size, size_t fragment2_size,
                                         struct pktbuf *pktbuf)
{
    uint8_t tag = MCTP_HDR_FLAG_TO | get_tag();
    uint8_t flags_seq_tag;

    flags_seq_tag = MCTP_HDR_FLAG_SOM | (get_sequence() << MCTP_HDR_SEQ_SHIFT) | tag;
    receive_one_fragment(binding, payload, fragment1_size, flags_seq_tag, pktbuf);

    flags_seq_tag = MCTP_HDR_FLAG_EOM | (get_sequence() << MCTP_HDR_SEQ_SHIFT) | tag;
    receive_one_fragment(binding, payload + fragment1_size, fragment2_size, flags_seq_tag, pktbuf);
}
...
static void mctp_core_test_simple_rx()
{
    ...
    receive_two_fragment_message(binding, test_payload, MCTP_BTU, MCTP_BTU, &pktbuf);
...
static void mctp_core_test_receive_equal_length_fragments()
{
    ...
    flags_seq_tag = MCTP_HDR_FLAG_SOM | (get_sequence() << MCTP_HDR_SEQ_SHIFT) | tag;
    receive_one_fragment(binding, test_payload, MCTP_BTU, flags_seq_tag, &pktbuf);

    flags_seq_tag = (get_sequence() << MCTP_HDR_SEQ_SHIFT) | tag;
    receive_one_fragment(binding, test_payload + MCTP_BTU, MCTP_BTU, flags_seq_tag, &pktbuf);

    flags_seq_tag = MCTP_HDR_FLAG_EOM | (get_sequence() << MCTP_HDR_SEQ_SHIFT) | tag;
    receive_one_fragment(binding, test_payload + (2 * MCTP_BTU), MCTP_BTU, flags_seq_tag, &pktbuf);
...
static void mctp_core_test_receive_unexpected_smaller_middle_fragment()
{
    ...
    flags_seq_tag = (get_sequence() << MCTP_HDR_SEQ_SHIFT) | tag;
    receive_one_fragment(binding, test_payload + MCTP_BTU, MCTP_BTU - 1, flags_seq_tag, &pktbuf);
...
static void mctp_core_test_receive_smaller_end_fragment()
{
    ...
    flags_seq_tag = MCTP_HDR_FLAG_EOM | (get_sequence() << MCTP_HDR_SEQ_SHIFT) | tag;
    receive_one_fragment(binding, test_payload + (2 * MCTP_BTU), end_frag_size, flags_seq_tag, &pktbuf);

    assert(test_param.seen);
    assert(test_param.message_size == (size_t) (2 * MCTP_BTU + end_frag_size));
...
static void mctp_core_test_drop_large_fragments()
{
	...
	* 2nd fragment of SIZE_MAX */
    ...
    receive_two_fragment_message(binding, test_payload, MCTP_BTU, SIZE_MAX - sizeof(struct mctp_hdr), &pktbuf);
...
static void mctp_core_test_rx_with_tag()
{
    ...
    flags_seq_tag = MCTP_HDR_FLAG_SOM | MCTP_HDR_FLAG_EOM | (1 << MCTP_HDR_TO_SHIFT) | tag;
    receive_one_fragment(binding, test_payload, MCTP_BTU, flags_seq_tag, &pktbuf);
...
static void mctp_core_test_rx_with_tag_multifragment()
{
    ...
    flags_seq_tag = MCTP_HDR_FLAG_SOM | (get_sequence() << MCTP_HDR_SEQ_SHIFT) | (1 << MCTP_HDR_TO_SHIFT) | tag;
    receive_one_fragment(binding, test_payload, MCTP_BTU, flags_seq_tag, &pktbuf);

    flags_seq_tag = (get_sequence() << MCTP_HDR_SEQ_SHIFT) | (1 << MCTP_HDR_TO_SHIFT) | tag;
    receive_one_fragment(binding, test_payload, MCTP_BTU, flags_seq_tag, &pktbuf);

    flags_seq_tag = MCTP_HDR_FLAG_EOM | (get_sequence() << MCTP_HDR_SEQ_SHIFT) | (1 << MCTP_HDR_TO_SHIFT) | tag;
    receive_one_fragment(binding, test_payload, MCTP_BTU, flags_seq_tag, &pktbuf);
...
int main(void)
{
    ...
    {
        mctp_prlog(MCTP_LOG_DEBUG, "begin: %s", mctp_core_tests[i].name);
