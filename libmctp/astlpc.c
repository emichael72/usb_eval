...
#define binding_to_astlpc(b) container_of(b, struct mctp_binding_astlpc, binding)

#define astlpc_prlog(ctx, lvl, fmt, ...)                                            \
    do                                                                              \
    {                                                                               \
        bool __bmc = ((ctx)->mode == MCTP_BINDING_ASTLPC_MODE_BMC);                 \
        mctp_prlog(lvl, pr_fmt("%s: " fmt), __bmc ? "bmc" : "host", ##__VA_ARGS__); \
    } while ( 0 )

#define astlpc_prerr(ctx, fmt, ...)    astlpc_prlog(ctx, MCTP_LOG_ERR, fmt, ##__VA_ARGS__)
#define astlpc_prwarn(ctx, fmt, ...)   astlpc_prlog(ctx, MCTP_LOG_WARNING, fmt, ##__VA_ARGS__)
#define astlpc_prnotice(ctx, fmt, ...) astlpc_prlog(ctx, MCTP_LOG_NOTICE, fmt, ##__VA_ARGS__)
#define astlpc_prinfo(ctx, fmt, ...)   astlpc_prlog(ctx, MCTP_LOG_INFO, fmt, ##__VA_ARGS__)
#define astlpc_prdebug(ctx, fmt, ...)  astlpc_prlog(ctx, MCTP_LOG_DEBUG, fmt, ##__VA_ARGS__)
...
static inline int mctp_astlpc_kcs_write(struct mctp_binding_astlpc *astlpc, enum mctp_binding_astlpc_kcs_reg reg, uint8_t val)
{
    return astlpc->ops.kcs_write(astlpc->ops_data, reg, val);
}

static inline int mctp_astlpc_kcs_read(struct mctp_binding_astlpc *astlpc, enum mctp_binding_astlpc_kcs_reg reg, uint8_t *val)
{
    return astlpc->ops.kcs_read(astlpc->ops_data, reg, val);
}

static inline int mctp_astlpc_lpc_write(struct mctp_binding_astlpc *astlpc, const void *buf, long offset, size_t len)
{
...
static inline int mctp_astlpc_lpc_read(struct mctp_binding_astlpc *astlpc, void *buf, long offset, size_t len)
{
    astlpc_prdebug(astlpc, "%s: %zu bytes from 0x%lx", __func__, len, offset);
...
static void mctp_astlpc_kcs_print_status_write(struct mctp_binding_astlpc *astlpc, uint8_t status)
{
    astlpc_prnotice(astlpc, "Binding state is 0x%hhx: BMC %s, Channel %s, OBF %s", status, status & KCS_STATUS_BMC_READY ? "active" : "inactive",
                    status & KCS_STATUS_CHANNEL_ACTIVE ? "active" : "inactive", status & KCS_STATUS_OBF ? "preserved" : "cleared");
}

static int mctp_astlpc_kcs_set_status(struct mctp_binding_astlpc *astlpc, uint8_t status)
{
    uint8_t data;
    int     rc;
...
static int mctp_astlpc_layout_read(struct mctp_binding_astlpc *astlpc, struct mctp_astlpc_layout *layout)
{
...
static int mctp_astlpc_layout_write(struct mctp_binding_astlpc *astlpc, struct mctp_astlpc_layout *layout)
{
    uint32_t rx_size_be;
    ...
    if ( astlpc->mode == MCTP_BINDING_ASTLPC_MODE_BMC )
    {
        ...
        return mctp_astlpc_lpc_write(astlpc, &hdr.layout, offsetof(struct mctp_lpcmap_hdr, layout), sizeof(hdr.layout));
    }
    ...
    return mctp_astlpc_lpc_write(astlpc, &rx_size_be, offsetof(struct mctp_lpcmap_hdr, layout.rx_size), sizeof(rx_size_be));
}

static bool mctp_astlpc_buffer_validate(const struct mctp_binding_astlpc *astlpc, const struct mctp_astlpc_buffer *buf, const char *name)
{
    /* Check for overflow */
    if ( buf->offset + buf->size < buf->offset )
    {
        mctp_prerr("%s packet buffer parameters overflow: offset: 0x%" PRIx32 ", size: %" PRIu32, name, buf->offset, buf->size);
        return false;
    }
    ...
    {
        mctp_prerr("%s packet buffer parameters exceed %uM window size: offset: 0x%" PRIx32 ", size: %" PRIu32, name, (LPC_WIN_SIZE / (1024 * 1024)),
                   buf->offset, buf->size);
        return false;
    }
    ...
    {
        mctp_prerr("%s packet buffer too small: Require %" PRIu32 " bytes to support the %u byte baseline transmission unit, found %" PRIu32, name,
                   astlpc->proto->packet_size(MCTP_PACKET_SIZE(MCTP_BTU)), MCTP_BTU, buf->size);
        return false;
    }
    ...
    {
        mctp_prerr("%s packet buffer overlaps control region {0x%" PRIx32 ", %" PRIu32 "}: Rx {0x%" PRIx32 ", %" PRIu32 "}", name, 0U, control_size,
                   buf->offset, buf->size);
        return false;
    }
...
static bool mctp_astlpc_layout_validate(const struct mctp_binding_astlpc *astlpc, const struct mctp_astlpc_layout *layout)
{
    ...
    if ( (rx->offset <= tx->offset && rx->offset + rx->size > tx->offset) || (tx->offset <= rx->offset && tx->offset + tx->size > rx->offset) )
    {
        mctp_prerr("Rx and Tx packet buffers overlap: Rx {0x%" PRIx32 ", %" PRIu32 "}, Tx {0x%" PRIx32 ", %" PRIu32 "}", rx->offset, rx->size, tx->offset,
                   tx->size);
        return false;
    }
...
static int mctp_astlpc_init_bmc(struct mctp_binding_astlpc *astlpc)
{
    ...
    astlpc->layout.rx.offset = astlpc->layout.tx.offset + astlpc->layout.tx.size;
...
{
    struct mctp_binding_astlpc *astlpc = container_of(b, struct mctp_binding_astlpc, binding);
...
static bool mctp_astlpc_validate_version(uint16_t bmc_ver_min, uint16_t bmc_ver_cur, uint16_t host_ver_min, uint16_t host_ver_cur)
{
    if ( ! (bmc_ver_min && bmc_ver_cur && host_ver_min && host_ver_cur) )
    {
        mctp_prerr("Invalid version present in [%" PRIu16 ", %" PRIu16 "], [%" PRIu16 ", %" PRIu16 "]", bmc_ver_min, bmc_ver_cur, host_ver_min, host_ver_cur);
        return false;
    }
    else if ( bmc_ver_min > bmc_ver_cur )
    {
        mctp_prerr("Invalid bmc version range [%" PRIu16 ", %" PRIu16 "]", bmc_ver_min, bmc_ver_cur);
        return false;
    }
    else if ( host_ver_min > host_ver_cur )
    {
        mctp_prerr("Invalid host version range [%" PRIu16 ", %" PRIu16 "]", host_ver_min, host_ver_cur);
        return false;
    }
    else if ( (host_ver_cur < bmc_ver_min) || (host_ver_min > bmc_ver_cur) )
    {
        mctp_prerr("Unable to satisfy version negotiation with ranges [%" PRIu16 ", %" PRIu16 "] and [%" PRIu16 ", %" PRIu16 "]", bmc_ver_min, bmc_ver_cur,
...
static int mctp_astlpc_negotiate_layout_host(struct mctp_binding_astlpc *astlpc)
{
    ...
    {
        astlpc_prerr(astlpc, "BMC provided invalid buffer layout: Rx {0x%" PRIx32 ", %" PRIu32 "}, Tx {0x%" PRIx32 ", %" PRIu32 "}", layout.rx.offset,
                     layout.rx.size, layout.tx.offset, layout.tx.size);
        return -EINVAL;
    }

    astlpc_prinfo(astlpc, "Desire an MTU of %" PRIu32 " bytes", astlpc->requested_mtu);
    ...
    if ( ! mctp_astlpc_layout_validate(astlpc, &layout) )
    {
        astlpc_prerr(astlpc, "Generated invalid buffer layout with size %" PRIu32 ": Rx {0x%" PRIx32 ", %" PRIu32 "}, Tx {0x%" PRIx32 ", %" PRIu32 "}", sz,
                     layout.rx.offset, layout.rx.size, layout.tx.offset, layout.tx.size);
        return -EINVAL;
    }

    astlpc_prinfo(astlpc, "Requesting MTU of %" PRIu32 " bytes", astlpc->requested_mtu);

    return mctp_astlpc_layout_write(astlpc, &layout);
}

static uint16_t mctp_astlpc_negotiate_version(uint16_t bmc_ver_min, uint16_t bmc_ver_cur, uint16_t host_ver_min, uint16_t host_ver_cur)
{
    if ( ! mctp_astlpc_validate_version(bmc_ver_min, bmc_ver_cur, host_ver_min, host_ver_cur) )
...
static int mctp_astlpc_init_host(struct mctp_binding_astlpc *astlpc)
{
    ...
    negotiated = mctp_astlpc_negotiate_version(bmc_ver_min, bmc_ver_cur, ASTLPC_VER_MIN, ASTLPC_VER_CUR);
    if ( ! negotiated )
    {
    ...
    mctp_astlpc_lpc_write(astlpc, &ver_min_be, offsetof(struct mctp_lpcmap_hdr, host_ver_min), sizeof(ver_min_be));

    mctp_astlpc_lpc_write(astlpc, &ver_cur_be, offsetof(struct mctp_lpcmap_hdr, host_ver_cur), sizeof(ver_cur_be));
...
static bool __mctp_astlpc_kcs_ready(struct mctp_binding_astlpc *astlpc, uint8_t status, bool is_write)
{
    bool    is_bmc;
    bool    ready_state;
    uint8_t flag;
...
static inline bool mctp_astlpc_kcs_read_ready(struct mctp_binding_astlpc *astlpc, uint8_t status)
{
    return __mctp_astlpc_kcs_ready(astlpc, status, false);
}

static inline bool mctp_astlpc_kcs_write_ready(struct mctp_binding_astlpc *astlpc, uint8_t status)
{
    return __mctp_astlpc_kcs_ready(astlpc, status, true);
}

static int mctp_astlpc_kcs_send(struct mctp_binding_astlpc *astlpc, enum mctp_astlpc_cmd data)
{
    uint8_t status;
    int     rc;
...
static int mctp_binding_astlpc_tx(struct mctp_binding *b, struct mctp_pktbuf *pkt)
{
    ...
    astlpc_prdebug(astlpc, "%s: Transmitting %" PRIu32 "-byte packet (%hhu, %hhu, 0x%hhx)", __func__, len, hdr->src, hdr->dest, hdr->flags_seq_tag);

    if ( len > astlpc->proto->body_size(astlpc->layout.tx.size) )
    {
        astlpc_prwarn(astlpc, "invalid TX len %" PRIu32 ": %" PRIu32, len, astlpc->proto->body_size(astlpc->layout.tx.size));
        return -EMSGSIZE;
    }
    ...
    len_be = htobe32(len);
    mctp_astlpc_lpc_write(astlpc, &len_be, astlpc->layout.tx.offset, sizeof(len_be));
...
static uint32_t mctp_astlpc_calculate_mtu(struct mctp_binding_astlpc *astlpc, struct mctp_astlpc_layout *layout)
{
...
static int mctp_astlpc_negotiate_layout_bmc(struct mctp_binding_astlpc *astlpc)
{
    ...
    if ( mctp_astlpc_layout_validate(astlpc, &pending) )
    {
        ...
        astlpc_prinfo(astlpc, "Negotiated an MTU of %" PRIu32 " bytes", mtu);
    }
    else
    {
...
static void mctp_astlpc_init_channel(struct mctp_binding_astlpc *astlpc)
{
    ...
    /* Version negotiation */
    negotiated = mctp_astlpc_negotiate_version(ASTLPC_VER_MIN, ASTLPC_VER_CUR, be16toh(hdr.host_ver_min), be16toh(hdr.host_ver_cur));
    ...
    mctp_astlpc_lpc_write(astlpc, &negotiated_be, offsetof(struct mctp_lpcmap_hdr, negotiated_ver), sizeof(negotiated_be));
    ...
    if ( negotiated > 0 )
    {
        astlpc_prinfo(astlpc, "Negotiated binding version %" PRIu16, negotiated);
    ...
    mctp_binding_set_tx_enabled(&astlpc->binding, status & KCS_STATUS_CHANNEL_ACTIVE);
}
...
static void mctp_astlpc_rx_start(struct mctp_binding_astlpc *astlpc)
{
	 ...
	 */
    mctp_astlpc_lpc_read(astlpc, mctp_pktbuf_hdr(pkt), astlpc->layout.rx.offset + 4, packet);
...
static int mctp_astlpc_finalise_channel(struct mctp_binding_astlpc *astlpc)
{
    ...
    rc = mctp_astlpc_lpc_read(astlpc, &negotiated, offsetof(struct mctp_lpcmap_hdr, negotiated_ver), sizeof(negotiated));
    if ( rc < 0 )
        return rc;
    ...
    if ( negotiated == ASTLPC_VER_BAD || negotiated < ASTLPC_VER_MIN || negotiated > ASTLPC_VER_CUR )
    {
    ...
    if ( negotiated >= 2 )
        astlpc->binding.pkt_size = astlpc->proto->body_size(astlpc->layout.tx.size);
...
static int mctp_astlpc_update_channel(struct mctp_binding_astlpc *astlpc, uint8_t status)
{
    uint8_t updated;
    int     rc = 0;
    ...
    astlpc_prdebug(astlpc, "%s: status: 0x%x, update: 0x%x", __func__, status, updated);
    ...
    if ( astlpc->proto->version == 0 || updated & KCS_STATUS_CHANNEL_ACTIVE )
    {
        bool enable;
...
int mctp_astlpc_poll(struct mctp_binding_astlpc *astlpc)
{
    ...
    if ( ! astlpc->proto->version && ! (data == cmd_initialise || data == cmd_dummy_value) )
    {
        astlpc_prwarn(astlpc, "Invalid message for binding state: 0x%x", data);
        return 0;
    }
    ...
    switch ( data )
    {
        ...
        case cmd_rx_complete:
            ...
            {
                astlpc_prerr(astlpc, "Protocol error: Invalid Tx buffer state for event %d: %d\n", data, astlpc->layout.tx.state);
                return 0;
            }
...
static struct mctp_binding_astlpc *__mctp_astlpc_init(uint8_t mode, uint32_t mtu)
{
    struct mctp_binding_astlpc *astlpc;

    assert((mode == MCTP_BINDING_ASTLPC_MODE_BMC) || (mode == MCTP_BINDING_ASTLPC_MODE_HOST));
    ...
    astlpc->binding.pkt_size    = MCTP_PACKET_SIZE(mtu > MCTP_BTU ? mtu : MCTP_BTU);
...
struct mctp_binding_astlpc *mctp_astlpc_init(uint8_t mode, uint32_t mtu, void *lpc_map, const struct mctp_binding_astlpc_ops *ops, void *ops_data)
{
    struct mctp_binding_astlpc *astlpc;

    if ( ! (mode == MCTP_BINDING_ASTLPC_MODE_BMC || mode == MCTP_BINDING_ASTLPC_MODE_HOST) )
    {
...
struct mctp_binding_astlpc *mctp_astlpc_init_ops(const struct mctp_binding_astlpc_ops *ops, void *ops_data, void *lpc_map)
{
    return mctp_astlpc_init(MCTP_BINDING_ASTLPC_MODE_BMC, MCTP_BTU, lpc_map, ops, ops_data);
}
...
{
    struct aspeed_lpc_ctrl_mapping map = {.window_type = ASPEED_LPC_CTRL_WINDOW_MEMORY,
    ...
    astlpc_prinfo(astlpc, "Configuring FW2AHB to map reserved memory at 0x%08x for 0x%x in the LPC FW cycle address-space", map.addr, map.size);
    ...
    lpc_map_base = mmap(NULL, map.size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
...
static int mctp_astlpc_init_fileio_kcs(struct mctp_binding_astlpc *astlpc, const char *kcs_path)
{
...
static int __mctp_astlpc_fileio_kcs_read(void *arg, enum mctp_binding_astlpc_kcs_reg reg, uint8_t *val)
{
...
static int __mctp_astlpc_fileio_kcs_write(void *arg, enum mctp_binding_astlpc_kcs_reg reg, uint8_t val)
{
    struct mctp_binding_astlpc *astlpc = arg;
    off_t                       offset = reg;
    int                         rc;
...
int mctp_astlpc_init_pollfd(struct mctp_binding_astlpc *astlpc, struct pollfd *pollfd)
{
    bool release;
    ...
    release = astlpc->layout.rx.state == buffer_state_prepared || astlpc->layout.tx.state == buffer_state_prepared;
...
#else
struct mctp_binding_astlpc *mctp_astlpc_init_fileio(const char *kcs_path __unused)
{
    mctp_prlog(MCTP_LOG_ERR, "%s: Missing support for file IO", __func__);
    return NULL;
}

int mctp_astlpc_init_pollfd(struct mctp_binding_astlpc *astlpc __unused, struct pollfd *pollfd __unused)
{
    mctp_prlog(MCTP_LOG_ERR, "%s: Missing support for file IO", __func__);
    return -1;
}
