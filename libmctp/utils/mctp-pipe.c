...
static void rx_message(uint8_t eid __unused, bool tag_owner __unused, uint8_t msg_tag __unused, void *data __unused, void *msg, size_t len)
{
    ssize_t rc;
...
int main(void)
{
    ...
    for ( ;; )
    {
        ...
        if ( n > 2 && pollfds[2].revents )
        {
            ...
            {
                mctp_message_tx(mctp[0], eids[1], MCTP_MESSAGE_TO_SRC, 0, buf, rc);
            }
        }
