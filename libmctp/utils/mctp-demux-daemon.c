...
static void client_remove_inactive(struct ctx *ctx)
{
    ...
    for ( i = 0; i < ctx->n_clients; i++ )
    {
        ...
        ctx->clients = realloc(ctx->clients, ctx->n_clients * sizeof(*ctx->clients));
    }
}
...
static int binding_null_init(struct mctp *mctp __unused, struct binding *binding __unused, mctp_eid_t eid __unused, int n_params, char *const *params __unused)
{
    if ( n_params != 0 )
    {
...
static int binding_serial_init_pollfd(struct binding *binding, struct pollfd *pollfd)
{
...
static int binding_astlpc_init_pollfd(struct binding *binding, struct pollfd *pollfd)
{
...
static int socket_process(struct ctx *ctx)
{
    ...
    ctx->n_clients++;
    ctx->clients = realloc(ctx->clients, ctx->n_clients * sizeof(struct client));
...
static int client_process_recv(struct ctx *ctx, int idx)
{
    ...
    if ( ctx->pcap.socket.path )
        capture_socket(ctx->pcap.socket.dumper, ctx->buf, rc, MCTP_MESSAGE_CAPTURE_OUTGOING, eid);

    if ( ctx->verbose )
        fprintf(stderr, "client[%d] sent message: dest 0x%02x len %d\n", idx, eid, rc - 1);

    if ( eid == ctx->local_eid )
        rx_message(eid, MCTP_MESSAGE_TO_DST, 0, ctx, ctx->buf + 1, rc - 1);
    else
...
static int run_daemon(struct ctx *ctx)
{
    ...
    for ( ;; )
    {
        if ( clients_changed )
        {
            ...
            ctx->pollfds = realloc(ctx->pollfds, (ctx->n_clients + FD_NR) * sizeof(struct pollfd));
        ...
        if ( ctx->binding->init_pollfd )
            ctx->binding->init_pollfd(ctx->binding, &ctx->pollfds[FD_BINDING]);
        ...
        if ( ctx->pollfds[FD_SIGNAL].revents )
        {
            ...
            if ( got == sizeof(si) )
            {
                warnx("Received %s, quitting", strsignal(si.ssi_signo));
                rc = 0;
                break;
            }
            else
            {
                warnx("Unexpected read result for signalfd: %d", rc);
...
static void usage(const char *progname)
{
    ...
    for ( i = 0; i < ARRAY_SIZE(bindings); i++ ) fprintf(stderr, "  %s\n", bindings[i].name);
}
...
int main(int argc, char *const *argv)
{
    ...
    if ( ctx->pcap.binding.path )
    {
        ...
        mctp_set_capture_handler(ctx->mctp, capture_binding, ctx->pcap.binding.dumper);
    }
