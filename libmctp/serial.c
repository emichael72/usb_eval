...
#define binding_to_serial(b) container_of(b, struct mctp_binding_serial, binding)
...
static int mctp_binding_serial_tx(struct mctp_binding *b, struct mctp_pktbuf *pkt)
{
    ...
    if ( ! serial->tx_fn )
        return mctp_write_all(mctp_serial_write, serial->fd, &serial->txbuf[0], len);

    return mctp_write_all(serial->tx_fn, serial->tx_fn_data, &serial->txbuf[0], len);
}

static void mctp_serial_finish_packet(struct mctp_binding_serial *serial, bool valid)
{
...
static void mctp_serial_start_packet(struct mctp_binding_serial *serial, uint8_t len)
{
...
static void mctp_rx_consume_one(struct mctp_binding_serial *serial, uint8_t c)
{
    ...
    assert(! pkt == (serial->rx_state == STATE_WAIT_SYNC_START || serial->rx_state == STATE_WAIT_REVISION || serial->rx_state == STATE_WAIT_LEN));
    ...
    switch ( serial->rx_state )
    {
        ...
        case STATE_WAIT_REVISION:
			 ...
			 * STATE_WAIT_REVISION
			 */
                mctp_prdebug("Received serial framing flag 0x%02x while waiting"
        ...
        case STATE_WAIT_SYNC_END:
            ...
            {
                valid = false;
                mctp_prdebug("invalid fcs : 0x%04x, expect 0x%04x", serial->rx_fcs, serial->rx_fcs_calc);
            }
...
static void mctp_rx_consume(struct mctp_binding_serial *serial, const void *buf, size_t len)
{
    size_t i;

    for ( i = 0; i < len; i++ ) mctp_rx_consume_one(serial, *(uint8_t *) (buf + i));
}
...
int mctp_serial_init_pollfd(struct mctp_binding_serial *serial, struct pollfd *pollfd)
{
    pollfd->fd     = serial->fd;
...
int mctp_serial_open_path(struct mctp_binding_serial *serial, const char *device)
{
...
void mctp_serial_set_tx_fn(struct mctp_binding_serial *serial, mctp_serial_tx_fn fn, void *data)
{
    serial->tx_fn      = fn;
    serial->tx_fn_data = data;
}

int mctp_serial_rx(struct mctp_binding_serial *serial, const void *buf, size_t len)
{
